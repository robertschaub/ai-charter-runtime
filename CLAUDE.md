...
- When loaded, the **PreToolUse hook** in [.claude/settings.json](.claude/settings.json) runs the shared Git safety policy in `tooling/git-safety-hook.cjs`, mirrored for Codex in [.codex/hooks.json](.codex/hooks.json). It is a backstop only — never delegate destructive git to a subagent.
...
- Keys live only in gitignored `.env.local`. The probe and all tooling must never print or persist them. Do not run live probes, generate or rotate keys, or sign model cards without explicit maintainer approval; normal checks are `npm run typecheck`, `npm test`, and, for card changes, `npm run cards:verify`.

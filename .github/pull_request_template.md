...
- [ ] Gate invariants preserved: model output never authorizes; signals have no path to `allow`; ambiguity or missing authority fails closed; the current orchestrator boundary in `AGENTS.md` is preserved; every consequential effect needs a valid single-use commit token.
